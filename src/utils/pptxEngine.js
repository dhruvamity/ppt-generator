...
export const generatePPTX = async (config, finalQuestions, themeId, layoutId = 'modern-sidebar') => {
    ...
    const buildModernSidebarQuestion = (slide, q) => {
        ...
        let textObjects = [{ text: q.qText, options: { color: theme.textWhite, fontSize: 14, fontFace: 'Arial' } }];
        if (q.options) {
            textObjects.push({ text: '\n\n', options: { fontSize: 14 } });
            let spacedOptions = q.options.replace(/\n+/g, '\n\n');
            textObjects.push({ text: spacedOptions, options: { color: theme.cyan, fontSize: 14, fontFace: 'Arial' } });
        }
        slide.addText(textObjects, { x: 0.8, y: 0.1, w: 8.1, h: 5.0, align: 'left', valign: 'top' });
    };
    ...
    const buildSplitFocusQuestion = (slide, q) => {
        ...
        if (q.options) {
            textObjects.push({ text: '\n\n', options: { fontSize: 14 } });
            let spacedOptions = q.options.replace(/\n+/g, '\n\n');
            textObjects.push({ text: spacedOptions, options: { color: theme.gold, fontSize: 14, fontFace: 'Arial' } });
        }
        slide.addText(textObjects, { x: 0.8, y: 0.1, w: 8.1, h: 5.0, align: 'left', valign: 'top' });
    };
